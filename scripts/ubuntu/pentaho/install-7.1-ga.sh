...
FILE=/host/builds/business-analytics/pentaho-business-analytics-7.1.0.0-12-x64.bin
...
if [ -f "$FILE" ]; then
  ...
  if curl --head --silent --fail $BUILD_HOST 2> /dev/null; then
    ...
    curl $BUILD_HOST/7.1.0.0/12/pentaho-business-analytics-7.1.0.0-12-x64.bin -o $FILE
  else
